...
def moa_principle(update, context):
    ...
    update.message.reply_text("Enter *References* (comma separated):")
    return MOA_REFERENCES

def moa_references(update, context):
    context.user_data["moa"]["references"] = [i.strip() for i in update.message.text.split(",")]
    update.message.reply_text("Enter *Reagents* (comma separated):")
    return MOA_REAGENTS

def moa_reagents(update, context):
    context.user_data["moa"]["reagents"] = [i.strip() for i in update.message.text.split(",")]
    update.message.reply_text("Enter *Standards* (comma separated):")
    return MOA_STANDARDS

def moa_standards(update, context):
    context.user_data["moa"]["standards"] = [i.strip() for i in update.message.text.split(",")]
...
def moa_equipment(update, context):
    context.user_data["moa"]["equipment"] = [i.strip() for i in update.message.text.split(",")]
...
moa_conv = ConversationHandler(
    entry_points=[
        ...
        # ⭐ FIXED BULLETPROOF BUTTON/KEYWORD MATCH ⭐
        MessageHandler(
            Filters.regex(
                r"(?i)(^|\s)(moa|🧪|method\s*of\s*analysis|method\s+analysis)(\s|$)"
            ),
            start_moa
        ),
    ],
